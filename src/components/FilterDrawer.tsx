...
const FilterDrawer: React.FC<FilterDrawerProps> = ({ isOpen, onClose, onApply, imageSrc, documentSize, mainCanvasRef }) => {
  ...
  const handleApply = async (mode: "applyCurrent" | "createNew") => {
    ...
    if (filter.startsWith("shader_")) {
      ...
      if (!filteredImage) {
        console.error("No filtered image available for shader filter.");
        return;
      }

      // Draw the filtered image from the WebGLFilterRenderer onto the mainCanvas
      ...
      const img = new Image();
      img.src = filteredImage; // Use the filteredImage generated by WebGLFilterRenderer
      ...
      img.onerror = () => {
        console.error("Failed to load filtered image for shader filter.");
      };
    } else {
