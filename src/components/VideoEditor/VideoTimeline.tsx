    ...
    <div className="h-full flex flex-col bg-gray-900 text-white">
      ...
      <div className="flex items-center justify-between p-4 bg-gray-800 border-b border-gray-700">
        <div className="flex items-center space-x-3">
          <button onClick={onPlayPause} className="flex items-center justify-center w-10 h-10 bg-blue-600 hover:bg-blue-700 rounded-full transition-colors">
            {isPlaying ? <PauseIcon className="w-5 h-5" /> : <PlayIcon className="w-5 h-5" />}
          </button>
          <button onClick={onStop} className="flex items-center justify-center w-10 h-10 bg-red-600 hover:bg-red-700 rounded-full transition-colors">
      ...
      <div className="flex items-center space-x-2 p-3 bg-gray-800 border-b border-gray-700">
        ...
        <button onClick={() => onAddTrack("image")} className="flex items-center px-3 py-1.5 bg-green-600 hover:bg-green-700 rounded text-sm transition-colors">
        ...
        <button onClick={() => onAddTrack("video")} className="flex items-center px-3 py-1.5 bg-blue-600 hover:bg-blue-700 rounded text-sm transition-colors">
          <PlusIcon className="w-4 h-4 mr-1" />
          Video
        </button>
      </div>
        ...
        <div className="flex-1 relative overflow-auto" ref={timelineRef} onClick={handleTimelineClick}>
          ...
          <div style={{ width: timelineWidth }}>
            {tracks.map((track, index) => (
              <div key={`track-${track.id}-${index}`} className="relative border-b border-gray-700" style={{ height: trackHeight }}>
                {/* Track Block */}
                <div
                ...
                >
                  ...
                  <div className="p-2 h-full flex items-center justify-between text-xs">
                    ...
                    {track.type === "audio" && track.volume !== undefined && (
                      <div className="text-white opacity-75">Vol: {Math.round((track.volume || 0) * 100)}%</div>
                    )}
      ...
      {showEffectsPanel && selectedTrack && (
        <div className="h-32 bg-gray-800 border-t border-gray-700 p-3">
          ...
          <div className="flex space-x-2">
            <button onClick={() => addTransition(selectedTrack)} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 rounded text-sm">
              Fade Transition
            </button>
            <button
              onClick={() => {
      ...
      {showKeyframesPanel && selectedTrack && (
        <div className="h-32 bg-gray-800 border-t border-gray-700 p-3">
          ...
          <div className="flex space-x-2">
            <button onClick={() => addPropertyKeyframe(selectedTrack, "opacity")} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 rounded text-sm">
              Opacity
            </button>
            <button onClick={() => addPropertyKeyframe(selectedTrack, "volume")} className="px-3 py-1.5 bg-purple-600 hover:bg-purple-700 rounded text-sm">
              Volume
            </button>
            <button onClick={() => addPropertyKeyframe(selectedTrack, "scale")} className="px-3 py-1.5 bg-green-600 hover:bg-green-700 rounded text-sm">
              Scale
            </button>
            <button onClick={() => addPropertyKeyframe(selectedTrack, "position")} className="px-3 py-1.5 bg-yellow-600 hover:bg-yellow-700 rounded text-sm">
              Position
            </button>
          </div>
        </div>
