  ...
  const play = useCallback(
    async (blob: Blob, fileId: string) => {
      try {
        ...
        const audioContext = (Tone.context as any).rawContext as AudioContext;
        ...
        const player = new (Tone as any).Player(buffer);
        playerRef.current = player;

        // Connect player to analyzer and destination
        const toneAnalyser = new (Tone as any).Analyser("fft", 128);
